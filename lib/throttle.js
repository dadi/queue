...
Throttle.prototype.throttleQueueMessage = function (message, failure, success) {
  var limits = message && this.findMessageLimits(message) || this.queue
...
Throttle.prototype.findMessageLimits = function (messageObject) {
  for (var i = 0; i < this.messages.length; i++) {
    var re = new RegExp(this.messages[i].regex, this.messages[i].regexOpts)
...
Throttle.prototype.pruneQueueHistory = function (limits) {
  ...
  var cutoffTime = Date.now() - (limits.value * modifier)
  for (var i = limits.history.length - 1; i >= 0; i--) {
